...
- **FC_LLM_MAX_CONCURRENCY**: Controls the maximum concurrent requests to the LLM API (default: 5). Useful to prevent rate limiting from the provider.
- **FC_DOMAIN_MAX_CONCURRENCY**: Controls the maximum concurrent HTTP requests to a single domain during fulltext fetching (default: 3). Prevents IP bans and 429 errors from target sites.
