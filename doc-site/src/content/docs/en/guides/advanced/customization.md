...
- **LOG_LEVEL**: (Optional) Configures the system logging level. Available options are `debug`, `info`, `warn`, `error`, and `trace`. (Default: `info` in production, `debug` in development).
