...
4.  **使用 (Use)** 生成的选择器在自定义食谱 (Custom Recipe) 中。
...
3.  **字段选择器 (Field Selectors)**：
    ...
    - **链接选择器 (Link Selector)**：点击 Pick 并选择包含文章 URL 的元素（通常是 `<a>` 标签）。
      > **提示：** 提取器非常智能！如果你选择的元素不是链接（如 `div` 或 `span`），它会自动在父级或子级中查找链接。如果没有找到链接，它会尝试使用文本内容（如果看起来像 URL）。(See [ea6c69e](https://github.com/Colin-XKL/FeedCraft/commit/ea6c69eb55e114ccf7c32c7cc100e5a6514ea70b))
