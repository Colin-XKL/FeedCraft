...
- **使用方法**:
  1. 导航至 **工具 > 系统运行状态**。
- **功能**:
  - **资源健康 (Resource Health)**: 查看食谱及其他组件的当前状态（健康、降级、暂停），包括连续失败次数。
  - **执行日志 (Execution Logs)**: 跟踪详细的执行历史、成功率以及每次运行的具体错误类型（例如：超时、网络错误、解析错误）。
  - **系统通知 (System Notifications)**: 查看关于资源状态转换的自动警报（例如当食谱降级时）。您还可以通过内置的 RSS 源 `/system/notifications/rss` 订阅这些警报。

:::tip
如果食谱反复失败并变为“暂停 (Paused)”状态，您可以在解决根本问题后，通过系统运行状态仪表板手动将其“恢复 (Resume)”。
:::
