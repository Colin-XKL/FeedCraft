...
- **注意**: 预览器默认使用 `proxy` 工艺，它只是简单地抓取 Feed 而不进行修改。如果提取过程中发生错误（如 browserless 解析失败），错误信息会清晰展示，而非返回通用的服务器错误。
...
- **功能**:
  - 生成所有依赖关系的树状视图。
  - **健康检查**: 自动检测丢失的依赖项（例如，指向已删除 FlowCraft 的 Recipe）。缺失组件会通过醒目的警告卡片和内联红色标签突出显示，以便快速识别。
