...
FeedCraft 在管理后台中提供了几个实用工具，帮助你管理和调试 Feed。

### RSS 预览 (RSS Viewer)

**RSS 预览** (实用工具 > RSS 预览) 允许你预览和验证任何 RSS Feed。

- **预览**: 输入 Feed URL 以用户友好的格式渲染其内容。
- **复制链接**: 预览列表中的每一项都包含一个 **复制链接** 按钮，方便快速获取原始文章链接。
- **视图模式**: 在“普通 (Normal)”（摘要）和“丰富 (Rich)”（完整内容）视图之间切换。

### Feed 对比 (Feed Compare)

**Feed 对比** 工具 (实用工具 > Feed 对比) 帮助你可视化 Craft（配方）应用于原始 Feed 后的变化。

- **对比**: 输入原始 Feed URL 并选择一个 Craft，即可查看原始内容与处理后输出的并排对比。
- **调试**: 用于验证你的转换规则（例如内容提取、翻译）是否按预期工作。
