...
1.  **配方 ID (Recipe ID)**：为此配方选择一个唯一的标识符（例如 `search-ai-news`）。
...
保存后，该配方将作为 **配方管理 (Recipes)** 中的配方存储。你可以在 **工作台 > Feed 源管理 > 配方管理** 中管理它。
