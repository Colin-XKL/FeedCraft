...
为了方便使用，你可以利用 **URL 生成器** 来轻松构建这些 URL。

- **独立生成器 (`/start.html`)**: 一个简化的工具，无需登录即可快速生成 URL。
- **管理后台 (仪表盘 > 快速开始)**: 功能完整的生成器，包含 **URL 解析模式**。

**URL 解析模式**
管理后台中的生成器支持“解析模式”。你可以粘贴一个现有的 FeedCraft URL，工具会反向解析出它使用的工艺（Craft）、原始来源 URL 以及其他参数。这对于调试或理解复杂的 FeedCraft 链接非常有用。
