...
- **去重 (Deduplicate)**：移除跨来源的重复文章。支持五种策略：
  ...
  - `by_id` — 按 GUID 精确匹配
  - `by_title` — 按标题精确匹配
  - `by_simhash` _（轻量）_ — 基于字符特征的字面模糊去重，通过比较标题和正文的文字相似度判断是否重复，无需 AI 服务。适合过滤转载、微调改写等字面高度相似的重复文章。阈值为**差异度**（`0.0` = 仅完全一致，`1.0` = 全部视为重复，默认 `0.05`，越小越严格）。
  - `by_embedding` _（精准）_ — 基于大模型语义向量的深度语义去重，理解文章含义而非仅比对文字，能识别"同义不同字"的内容，适合聚合不同媒体对同一事件的报道。需配置 Embedding API；服务不可用时自动降级（保留全部文章）。阈值为**余弦相似度**（`0.0`–`1.0`，默认 `0.9`，越高越严格）。
