...
- **去重 (Deduplicate)**：移除跨来源的重复文章。支持五种策略：
  ...
  - `by_id` — 按 GUID 精确匹配
  - `by_title` — 按标题精确匹配
  - `by_simhash` — 基于字符 2-gram SimHash 的字面模糊去重。差异度低于阈值的文章视为重复。阈值范围：`0.0`（仅完全一致）至 `1.0`（全部视为重复），默认 `0.05`。
  - `by_embedding` — 基于 LLM Embedding 的语义去重。余弦相似度达到或超过阈值的文章视为重复。阈值范围：`0.0` 至 `1.0`，默认 `0.9`。需配置 Embedding API；服务不可用时自动降级（不去重）。
