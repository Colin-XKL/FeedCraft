...
:::note
此工具专为 HTML 页面设计。如果你需要处理 JSON API，请使用 [从 JSON 生成 RSS](/zh/guides/json-to-rss/)。
:::
...
1.  在管理后台导航至 **工作台 > Feed 源生成 > 网页转 RSS**。

### 第一步：目标 URL (Target URL)

1.  输入你想要抓取的网页完整 URL（例如博客列表或新闻网站）。
2.  **增强模式 (Enhanced Mode)**：如果网站需要 JavaScript 来加载内容（使用无头浏览器），请启用此选项。
3.  **前置导航操作**（可选）：按顺序添加点击元素、等待元素或等待时间操作，例如先点击 `#tab-a`、`#tab-b`、`#tab-c`，再抓取最终页面。
4.  点击 **抓取并下一步 (Fetch and Next)**。
