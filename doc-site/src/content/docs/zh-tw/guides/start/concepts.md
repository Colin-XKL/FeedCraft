...
- **去重 (Deduplicate)**：移除跨來源的重複文章。支援五種策略：
  ...
  - `by_id` — 按 GUID 精確比對
  - `by_title` — 按標題精確比對
  - `by_simhash` _（輕量）_ — 基於字元特徵的字面模糊去重，透過比較標題與正文的文字相似度判斷是否重複，無需 AI 服務。適合過濾轉載、微調改寫等字面高度相似的重複文章。閾值為**差異度**（`0.0` = 僅完全一致，`1.0` = 全部視為重複，預設 `0.05`，越小越嚴格）。
  - `by_embedding` _（精準）_ — 基於大型語言模型語意向量的深度語意去重，理解文章含義而非僅比對文字，能識別「同義不同字」的內容，適合聚合不同媒體對同一事件的報導。需設定 Embedding API；服務不可用時自動降級（保留全部文章）。閾值為**餘弦相似度**（`0.0`–`1.0`，預設 `0.9`，越高越嚴格）。
