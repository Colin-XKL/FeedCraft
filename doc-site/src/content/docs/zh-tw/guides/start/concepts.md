...
- **去重 (Deduplicate)**：移除跨來源的重複文章。支援五種策略：
  ...
  - `by_id` — 按 GUID 精確比對
  - `by_title` — 按標題精確比對
  - `by_simhash` — 基於字元 2-gram SimHash 的字面模糊去重。差異度低於閾值的文章視為重複。閾值範圍：`0.0`（僅完全一致）至 `1.0`（全部視為重複），預設 `0.05`。
  - `by_embedding` — 基於 LLM Embedding 的語意去重。餘弦相似度達到或超過閾值的文章視為重複。閾值範圍：`0.0` 至 `1.0`，預設 `0.9`。需設定 Embedding API；服務不可用時自動降級（不去重）。
