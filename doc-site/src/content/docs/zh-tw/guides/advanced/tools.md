...
- **注意**: 預覽器默認使用 `proxy` 工藝，它只是簡單地抓取 Feed 而不進行修改。如果提取過程中發生錯誤（如 browserless 解析失敗），錯誤信息會清晰展示，而非返回通用的服務器錯誤。
...
- **功能**:
  - 生成所有依賴關係的樹狀視圖。
  - **健康檢查**: 自動檢測丟失的依賴項（例如，指向已刪除 FlowCraft 的 Recipe）。缺失組件會透過醒目的警告卡片和內聯紅色標籤突出顯示，以便快速識別。
