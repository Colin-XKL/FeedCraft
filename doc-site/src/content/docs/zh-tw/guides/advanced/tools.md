...
**Feed 對比** 工具允許你並排比較兩個 RSS Feed。這在調試 Craft Flow 時特別有幫助，可以直觀地看到處理步驟如何改變了原始 Feed。

- **位置**: **工具 (Tools) > Feed 對比**
- **使用方法**:
  1.  輸入原始 Feed 的 **來源連結**。
  2.  從下拉選單中選擇一個 **Craft Flow** (例如 `translate-title`)。
  3.  點擊 **對比**。
  4.  工具將在左側顯示「原始 Feed」，在右側顯示「處理後的 Feed」，方便你發現差異。

## 系統健康 (System Health)

**系統健康** 儀表板可視化展示了 FeedCraft 實例的內部依賴關係圖。它顯示了 Recipe (配方)、Flow (流程) 和 AtomCraft (原子) 之間的關係。

- **位置**: **工具 (Tools) > 系統健康** (或 Craft 依賴檢查)
- **用途**:
  - 驗證自訂 Recipe 的所有組件是否存在。
  - 檢測缺失的依賴項（例如，Recipe 引用了已刪除的 Flow）。
  - 識別可能導致問題的循環依賴。
- **使用方法**:
  1.  點擊 **開始分析**。
  2.  查看樹狀圖。健康的組件會標記其類型（Recipe, Flow, Atom）。缺失的組件將標記為紅色。
...
- **位置**: **儀表板 > 快速開始**
- **功能**:
  - **生成**: 選擇 Craft Flow 並輸入來源 URL，即可獲取訂閱連結。
  - **解析**: 貼上 FeedCraft URL 以查看其組成部分（使用的 Craft、原始來源等）。
