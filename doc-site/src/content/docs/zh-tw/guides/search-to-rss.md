...
在使用搜尋轉 RSS 功能之前，您需要在管理員設定中配置搜尋供應商。請參閱 [搜尋供應商配置指南](/zh-tw/guides/customization) 獲取設定說明。

FeedCraft 包含一個 **搜尋轉 RSS (Search to RSS)** 工具，允許你將搜尋查詢轉換為 RSS 訂閱源。這對於使用配置的搜尋供應商（例如 SearXNG, Bing, Google）追蹤新聞、話題或品牌提及非常有用。

## 如何使用

1.  在管理後台導航至 **工作台 > Feed 源生成 > 搜尋轉 RSS**。
...
1.  **配方 ID (Recipe ID)**：為此配方選擇一個唯一的識別碼（例如 `search-ai-news`）。
