export default {
  ...
  'settings.searchProvider.alert': '配置“搜索转 RSS”功能使用的搜索提供商。目前支持兼容 LiteLLM 的搜索代理。',
  'settings.searchProvider.provider': '提供商实现',
  'settings.searchProvider.provider.tooltip': "要使用的内部提供商逻辑。目前仅支持 'litellm'。",
  'settings.searchProvider.apiUrl': 'API URL',
  'settings.searchProvider.apiUrl.tooltip': '搜索提供商的基准 URL (例如 http://litellm-proxy:4000)。',
  'settings.searchProvider.apiKey': 'API Key',
  'settings.searchProvider.apiKey.tooltip': '如果提供商需要身份验证，则为可选 API 密钥。',
  'settings.searchProvider.toolName': '工具名称',
  'settings.searchProvider.toolName.tooltip': "要调用的具体工具名称 (例如 'google-search', 'brave-search', 'perplexity')。这会根据提供商附加到 URL 或在请求中传递。",
